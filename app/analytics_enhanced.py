    ...
    def _initialize_tables(self):
        ...
        try:
            with self.get_db() as conn:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_events_conversion ON analytics_events(conversion_goal_id)"
                )
            ...
            with self.get_db() as conn:
                ...
                if step_results:
                    ...
                    for i, step in enumerate(step_results):
                        if i > 0:
                            ...
                            step["drop_off_rate"] = (
                                round(drop_off / previous_sessions * 100, 2) if previous_sessions > 0 else 0
                            )

                        # Conversion rate from start
                        step["conversion_rate"] = (
                            round(step["sessions"] / initial_sessions * 100, 2) if initial_sessions > 0 else 0
                        )
                ...
                return {
                    ...
                    "overall_conversion_rate": (
                        round(step_results[-1]["sessions"] / step_results[0]["sessions"] * 100, 2)
                        if step_results and step_results[0]["sessions"] > 0
                        else 0
                    ),
                }
        except Exception as e:
