...
async def get_country_from_ip(ip: str) -> Optional[str]:
    if ip in ["127.0.0.1", "localhost", "::1"] or ip.startswith(("192.168.", "10.")):
        return "Lokal"
    try:
...
def save_optimized_image(file: UploadFile, upload_dir: Path) -> str:
    try:
        ...
        raise HTTPException(500, f"Bildfehler: {e}")
...
async def generate_social_card(settings):
    ...
    try:
        font_bold = ImageFont.truetype(str(FONTS_DIR / "Roboto-Bold.ttf"), 60)
    except Exception:
