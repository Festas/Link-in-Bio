...
async def main():
    ...
    save_social_stats_cache(platform="instagram", username=stats["profile"]["username"], stats_data=json.dumps(stats))
    ...
    if new_token:
        ...
        if social_env.exists():
            ...
            with open(social_env, "w") as f:
                f.write(
                    f"# Instagram API Credentials - Automatisch aktualisiert am {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                )
            ...
            try:
                ...
                if len(repo_parts) == 2:
                    ...
                    if success:
                        ...
                        print(
                            "Bitte aktualisiere das GitHub Secret 'INSTAGRAM_SECRET' manuell mit dem obigen Inhalt.\n"
                        )
                else:
